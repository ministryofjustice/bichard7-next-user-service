...
|----------------------------------|----------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `$ARGON2_HASH_LENGTH`            | `32`                                               | This is the length of the Argon2 hash function output in bytes. Note that the resulting hash is encoded with Base 64, so the digest will be ~1/3 longer.                                     |
| `$ARGON2_MEMORY_COST`            | `16384`                                            | The amount of memory to be used by the Argon2 hash function, in KiB. Note that large values for highly concurrent usage will cause starvation and thrashing if your system memory gets full. |
| `$ARGON2_PARALLELISM`            | `2`                                                | The amount of threads to compute the Argon2 hash on. Each thread has a memory pool with `ARGON2_MEMORY_COST` size. Note that changing it also changes the resulting hash.                    |
| `$ARGON2_TIME_COST  `            | `10`                                               | The time cost is the amount of passes (iterations) used by the Argon2 hash function. It increases hash strength at the cost of time required to compute.                                     |
