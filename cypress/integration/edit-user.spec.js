...
describe("Edit user", () => {
  ...
  it("should display correct user details when navigating to the edit user page", () => {
    ...
    cy.task("selectFromUsersTable", emailAddress).then((user) => {
      cy.task("selectFromGroupsTable", "user_id", user.id).then((groups) => {
        ...
        cy.get('[data-test="checkbox-user-groups"]').find('[data-test="checkbox-multiselect-checkboxes"]').find(`label[for="${currentUserGroups[0].id}"]`).should('have.text', currentUserGroups[0].name)
      })
    })
  })
  ...
  it("should update user correctly when updating user details", () => {
    ...
    cy.get('[data-test="checkbox-user-groups"]').find('[data-test="checkbox-multiselect-checkboxes"]').find(`input[name="B7ExceptionHandler_grp"]`).check()
    ...
    cy.task("selectFromUsersTable", "bichard01@example.com").then((user) => {
      cy.task("selectFromGroupsTable", "user_id", user.id).then((groups) => {
        ...
        cy.get('[data-test="checkbox-user-groups"]').find('[data-test="checkbox-multiselect-checkboxes"]').find(`[data-test="B7ExceptionHandler_grp"]`).contains(currentUserGroups[0].name)
      })
    })
  })
  ...
  it("should show the correct values for groups select input when on the edit user page", () => {
    ...
    cy.task("selectFromGroupsTable").then((groups) => {
      ...
      cy.get('[data-test="checkbox-user-groups"]').find('[data-test="checkbox-multiselect-checkboxes"]').find('input').should("have.length", currentUserGroups.length)
      cy.get('[data-test="checkbox-user-groups"]').find('[data-test="checkbox-multiselect-checkboxes"]').find('input').each(($el, index) => {
        console.log($el)
